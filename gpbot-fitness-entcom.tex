...
We have applied GP to generate the bots because it provides a good mechanism to create rules from scratch, giving the possibility to define very different, but still competitive, behaviours, as it is shown in the study of the produced rules (behavioural models). 
...
In these experiments, sub-tree crossover and 1-node mutation evolutionary operators have been used, following other researchers' proposals that have used these operators obtaining good results \cite{Esparcia2013GPunreal}. In this
...
However, as these values are not directly comparable, we have conducted a graphical study of the evolution of fitness on the runs.
...
This could be explained considering the way the fitness is computed, because Victory fitness tends to promote to its generated bots, obviously, just obtaining the victory. Area and Slope also take care of the number of ships generated during the battles, which may lead to a more `conservative' (less aggressive) behaviour. This would be tested in section \ref{subsubsec:conditions_actions}.
...
\subsubsection{Analysis of obtained Decision Trees}
\label{subsubsec:conditions_actions}
