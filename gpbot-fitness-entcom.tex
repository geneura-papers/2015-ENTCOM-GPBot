...
%
\begin{abstract}
% What problem do we try to solve? - JJ
Genetic Programming (GP) is a classical evolutionary technique used to
evolve expressions represented via a tree. % This is something you
                                % should say in an abstract? That is
                                % really the abstract of your work? 
In this paper we use GP % why? 
to automatically create the
behavioural engine, i.e. the set  of decision rules, that will guide
the actions of an autonomous agent (\textit{Bot}), to compete
adversarially playing the real-time strategy game Planet Wars. % why
                                % this game?
This algorithm uses evaluates every bot and evolves a population of
them taking the score obtained into account. % That's what GP does. So
                                % what?
%However, evaluation of every solution is not straightforward. Several
%elements should be taken into accout, such as... that is why in this
%paper... - JJ
The work presents a deep analysis on the performance of three %deep as
                                %compared to what? - JJ
% In previous work, we did an initial approach to, obtaining ... but
% in this one, we will try to prove... - JJ
different fitness functions, based one in the concept of victories, % why three? why victories? Why everything? - JJ
and the other two in dynamic information about the evaluation
matches.  % why? - JJ
The analysis tests several aspects of the bots, in addition of their
final performance on battles, such as the evolution of the behavioural
trees themselves or the influence of noise/uncertainty present in this
context (in games). 
% Antonio - TODO: Poner las conclusiones reales de este trabajo
%The experiments show that the three used fitness functions generate
%bots that outperform the optimized human-defined one, being the
%area-based fitness function the one that produces better results.
% What is the selling point? Using GP? Testing different fitnesses? Is
% GP important? Is any of the fitnesses? What are the conclusions and
% how can you generalize them? - JJ
\end{abstract}
...
parents. %This is not experimental setup, it is description of
...
% AntonioDEF - yo tampoco sabría justificarlo. ANTARES dí algo y lo traduce Fergu. ;D
 ...
 \caption{Evolution of the average age of every execution by fitness
   method. -- TO DO -- Un crecimiento constante de la edad indicaría
   que el algoritmo no está generando individuos que sean mejores que
   los anteriores, por lo que se habría ...} %don't use Spanish!!!!! 
...
There is still a final remark, which concerns to the percentage of
matches that end in a tie, as it can be seen in Table
\ref{tab:ties}. The \textit{Victory-based} fitness results in more
ties against bots of the same type than the other approaches. This can
be explained because they use less information to perform the
evolution, obtaining quite similar behaviours. %What? What does it
                                %have to do with that? - JJ
 This is also reinforced
by previous results in which the bots of this fitness seemed to
perform similarly, obtaining close number of victories and thus,
getting small standard deviation values. 
...
can create competitive bots for RTS games, and study the behaviour of
...
generated individuals (fighting against it). This bot was the best one
