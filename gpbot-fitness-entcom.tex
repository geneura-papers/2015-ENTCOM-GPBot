...
\begin{itemize}
\item A {\em victory-based fitness}, that only considers the number of battles won in the evaluation. %ANTARES: TambiÃ©n tiene en cuenta los turnos, pero para desempatar...
% Antonio - mejor decirlo cuando se describa en detalle, para no liar aquí. ;)
% Antares: Ok
...
% Antonio - Yo esto no lo mencionaría, si acaso en algún punto se puede hacer la referencia, pero sin decir que es una extensión, ya que esto acarrea más problemas que otra cosa. Al decir que es una extensión hay que aclarar bien en qué se extiende y justificar la necesidad de esta extensión. Los revisores ya lo saben y si lo piden lo ponemos. ;D
...
We have considered this game in some previous studies \cite{Genebot_CEC11,genebot-evo12,Co-Genebot_EVO2014}, mainly applying Genetic Algorithms for evolving (the parameters of) a behavioural engine previously defined by a human expert from scratch. 
...
We used GP in a previous approach for Planet Wars bots \cite{Garcia14Treedepth}, as a following step which avoided the strict limitations that the initial bots had, such as the fixed behavioural structure (a Finite State Machine) which just offers a few degrees of improvement, namely the evolved set of eight parameters.
...
This paper goes a bit further and analyses the evolution of the DTs themselves along generations, in order to show how do they change. Moreover, this study let us to identify the type of strategies that the best bots follow, i.e. extracting some generated knowledge that could help us to improve our skills as players of this game.

However, as it is usual when EAs are applied \cite{Jin2005303,QianYZ13}, and also due to the pseudostochastic nature of the games, the evolution of bots is sensitive to {\em uncertainty} or {\em noise} \cite{merelo14:noisy}, even in a `simple' game as Planet Wars \cite{Mora_noisy_jcst}.
...
In this paper we propose testing three different fitness functions, with the aim of checking, in addition to just their raw performance, which one if less sensitive to this kind of noise.
...
%FERGU: el "devoted to" no me mola xD Tampoco sÃ© si el fitness es el "main component". Ah, no usar el "we"
% Antonio - arreglado
% la funciÃ³n de fitness es muy importante, pero Â¿mucho mucho mÃ¡s que tipos de nodos que componen los Ã¡rboles?
% Â¿no quedarÃ­a mejor diciendo "...explains one of the main components of the..." ?     [pedro]
% AntonioDEF - hecho
...
% "Turn based fitness"  [pedro] FERGU3: arreglado. HabrÃ­a que llamarlo fitness basado en VICTORIAS, que es lo importante!!!!! (de hecho, el valor de las tablas muestra las VICTORIAS)
% ANTARES: De hecho en todos lados se llama asÃ­...
...
% AntonioDEF - Â¿cÃ³mo que 0 turns? Â¿CÃ³mo se va a ganar en 0 turnos?
% Antares - Es un mÃ¡ximo teÃ³rico, y por tanto inalcanzable en la realidad.
...
% AntonioDEF - Â¿dejamos hierachical o victories?
...
% Antonio - Al final creo que sÃ³lo Genebot %FERGU2: quitado
% Antares: Si
Maps used in each evaluation & map76, map69, map7, map11, map26 
% AntonioDEF - los mapas han sido esos, Â¿no?
% Antares: Si
\\ \hline
\end{tabular}
...
%FERGU3: ANTARES, confirma como has hecho esto ANTARES: Confirmo, se ha hecho la prueba en 4 mapas adicionales desconocidos y frente a oponentes desconocidos a su vez (los que han generado los mÃ©todos)
...
% AntonioDEF - la misma desv. tÃ­pica? huele a copy-paste mal hecho. XD
% Antares: Comprobado, habÃ­a un error en el copy paste, corregido.
...
%% DE AQUÃ PARA ABAJO ES TEXTO ANTIGUO DEL EVO QUE NO TIENE QUE VER CON ESTE ARTÃCULO

%FERGU: he borrado las tablas, que aquÃ­ no se usan. Dejo el resto comentado, por si querÃ©is "inspiraros". Vamos, que no se reutiliza nada del trabajo del Evostar
...
%ANTARES: Se me ha ocurrido que como trabajo futuro se podría estudiar las "condiciones" y "acciones" de forma junta. Es decir, estudiar si una acción concreta tiene como predileción un conjunto de condiciones para que cupla concreta. Puede ser costoso de computar (no se me ocurre una manera snecilla de hacerlo), pero croe que puede ser interesante al menos citarlo como futuro trabajo.
