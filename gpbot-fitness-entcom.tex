...
\title{Analyzing the Influence of the Fitness Function on Genetic Programming for the Automatic Generation of Bots for Real-Time Strategy Games}
%ANTARES: Cambiando el título para poner términos más SEO
...
% ESO no es un abstract. La primera frase debe introducir el problema
% que quieres resolver, como "Finding the global best strategy in a RTS game
% is a hard problem that can be generalized to logistics decision
% making and task allotment" 
Finding the global best strategy in a RTS (Real-Time Strategy) game
is a hard problem that can be generalized to logistics decision
making and task allotment. The automatic generation of autonomous agents (bots) to play this kind
of games is a very prolific research line, and different methods have been used in the past.

As it is impossible to know in
advance what kind of strategies will be the best in the most general
case, Genetic Programming, which searches the general decision tree space, has
been introduced and used successfully. However, it is not
straightforward what fitness function to use, since there is no consensual 
way to measure the quality of a bot. Thus, the objective of 
this paper is to demonstrate that using different fitness functions 
may generate bots with different behaviours, and therefore, that is convenient to choose a proper fitness 
depending on the specific scope.

% so what? - JJ 
%However, the techniques applied to do this usually must deal with uncertainty
%and real-time planning in order to control the game agents. 
% una frase muy directa indicando el problema general a resolver en
% este trabajo
% GP deals with uncertainty and real-time planning? - JJ
% FERGU: reescrito el abstract casi entero





%This work uses a Genetic Programming algorithm (GP) to create the
%behavioural engine of bots able to play an RTS. GP is a suitable method to generate bots to play a `simple' RTS game, Planet Wars.
% 1. It does not present. It has been used before.
% 2. You have mentinoed the issues before: uncertainty and real
% time. Can GP deal with this?
% Why don't you say something like "it is impossible to know in
% advance what kind of strategies will be the best in the most general
% case. So GP, which searches the general decision tree space, has
% been introduced and used succesfully. However, it is not
% straightforward what fitness function to use... - JJ
Three different
fitness functions, that measure different aspects of the game, are compared, % WHY? OH Why? Really, you have to TELL
                               % and JUSTIFY what the paper is about,
                               % you can't just shoot sentences like
                               % out of a buzzword-generated
                               % thing. You haven't mentioned fitness
                               % so far. Why is now important? Why is
                               % GP important? Why is Planet Wars
                               % important? Why are RTS games
                               % important? You have to justify every
                               % step of the way to make this paper
                               % unique, advancing the state of the
                               % art and relevant to the field at
                               % large. And why 3 and not 5 or 55?
                               % Does that exhaust the space of
                               % possible fitness functions? - JJ
and a detailed analysis of their performance has been conducted.
%, conducted a detailed analysis of their performance
%The aim is to deal with this uncertainty. % Why? Are they statistically distributed better? If
                  % you deal with that objective, you have to prove
                  % you have done so. 
% una frase muy directa indicando el resultado obtenido en este trabajo (el mas destacable)
%The results show that GP is a suitable method to generate bots to play
%a `simple' RTS game, Planet Wars.  % but that was already proved in
                                % another paper!!! JJ
% Moreover, it provides a mechanism to generate rules that enable to
% define very different behaviours. % You were interested in defining
                                % different behaviors? You should have
                                % said so before! And why are you
                                % interested in this thing? - JJ
% un par de frases indicando lo que aporta este resultado al estado del arte
This study analyses several aspects of three kinds of generated bots, in addition to their
final performance on battles, such as the evolution of the decision
trees themselves, or the influence on the results of noise/uncertainty
always present in the context of games. % Why????? Why are you
                                % interested in the evolution of the
                                % decision trees? - JJ
% una ultima frase poniendo el resultado en un ambito general
A victory-based fitness, which prioritizes the
number of victories, contributes to generate better bots,
than other functions based on the numerical aspects of the battles, 
such as the number of resources gathered, or the number of units generated,
obtaining also different values in the number of actions and decisions of the trees that model the bots.
Therefore, we can conclude that different behaviours can emerge from different fitness functions. 


 % that is not a conclusion. That is a result. The
            % conclusion would be what you deduce from that. PABLO: cambiado. Resultado->conclusion
\end{abstract}
% Antonio - he hecho una mejora de lo que proponía Pedro, atendiendo
% también a lo que sugería JJ. A ver si os gusta. ;)
% No, no me gusta nada - JJ
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   KEYWORDS   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\begin{keyword}
...
The result of this study can be useful to decide which fitness use in others games or scenarios, and also for experts, that can extract knowledge about what it is the priority goal to master the game, and what strategies are more profitable to reach these goals.
%ANTARES: Párrafo añadido. No sé si lo de <to master> está bien dicho en un entorno serio.
...
Genetic Programming (GP) \cite{GP_Koza92} is a type of EA where the individuals are represented as hierarchical structures (typically trees), which size and shape is not defined a priori, as in other evolutionary methods, evolving along the generations. Thus, GP performs a structural optimisation, which requires the use of specific genetic operators, focused on the management (and improvement) of this kind of structure.
...
(GAIC)\footnote{http://planetwars.aichallenge.org/}. 
 ...
 \end{center}
 \caption{Simulated screenshot of an early stage of a run in Planet Wars. White planets belong to the player, dark grey belong to the opponent, and light grey planets belong to no player. The triangles are fleets, and the numbers (in planets and triangles) represent the ships. The planet size means growth rate of the amount of ships in it (the bigger, the higher).}
...
% Una cosa, ¿no podemos hablar en español entre autores españoles?
% Vamos a ser pragmáticos.
% Why? Don't you want to speak in English? What if some other author
% comes in? - JJ

Therefore, the objective of GP applied to this game is to generate a reliable DT
% DT? - JJ
...
\cite{Mora_UnrealBots10,Esparcia2013GPunreal}.  % if that's the case,
                                % what is the point of this paper? - JJ

With respect to RTS games, it has not been extremely exploited, so
there are just a few applications, such as pathfinding
\cite{pathfinding_GP_RTS}, definition of tactics in an abstract
tactical game \cite{KeaveneyO09_GP_RTS}, and recently to the automatic generation of strategies for a bot \cite{Garcia15Starcraft}.
% Antonio - Fergu, mira a ver si quieres añadir algo a esto. ;)
In this paper, the aim is to apply GP inside a `simple' RTS, Planet
Wars, in order to define the whole behavioural engine for bot. %why? - JJ
 
Planet Wars, the game used in this work, has also been used by other researchers
as an experimental framework for agent testing \cite{ZiolkoK12_PW_Reasoning}.  
% references! - JJ
...
Severals authors along the time have worked on this game in some previous studies
...
% Por otra parte, ¿por qué hay que demostrar que es popular? Es un RTS
% simple y nos sirve de banco de pruebas, con eso creo que basta... 
% No, no basta. Tienes que tratar de ser lo más general posible. Si lo
% usas como banco de pruebas, tienes que decir a qué se puede
% generalizar - JJ

% Antonio - TODO: anonimizar referencias
We used GP in a previous approach for Planet Wars bots
\cite{Garcia14Treedepth_anonymous}, as a next step to those works, to avoid
...
parameters. % This sentence should be rewritten. - JJ
Thus, GP was applied to create the Decision Tree (DT) that the bots
used to decide on different courses of action during the game. 
% You have already presented GP for this particular game. You cannot
% say this paper is about "testing the validity of GP to create
% competitive bots". Please rewrite that - JJ
The
resulting bots were able to beat GeneBot, %GeneBot
                                %\cite{Genebot_CEC11},  
the initial bot improved by means of GAs.
...
In this paper we also propose testing three different fitness
functions,
% So this paper is about fitness functions in games and this is the
% only mention in the state of the art, right? - JJ
...
The Genetic Programming-based bot %or {\em GPBot} \cite{Garcia14Treedepth_anonymous} %
evolves a set of rules which, in turn, models a Decision Tree (DT).
...
% Antonio - TODO: anonimizar referencia FERGU: he quitado todos los anonimos porque no decimos que seamos nosotros
...
Both of them are based on the percentage of ships belonging to each player in every turn. They are normalised considering the total amount of ships in the game for that turn (including neutrals ships in neutral planets), so at the end of a simulated-game for each player there is a different {\em cloud of ships} with a value for every turn of the game.
...
\begin{equation}
\label{eq:alpha}
        \alpha = \frac{\sum_{i=1}^{n}(X_{i} - \bar{X_{i}})(Y_{i} - \bar{Y_{i}})}{\sum_{i=1}^{n}(X_{i} - \bar{X_{i}})^{2}} \in [-1,1]
\end{equation}
...
\begin{equation}
        area=\frac{\int_{0}^{t}\%ships(x)dx}{t} \in [0,1]
    \label{eq:area}
\end{equation}
...
A publicly available bot has been chosen for the experiments 
%\footnote{It can be downloaded from \url{https://github.com/deantares/genebot}}.
...
As it can be seen in the dispersion of the cloud of points, the noise if present in all the functions, but Slope shows a better tolerance to it, since the cloud is a bit narrower than the others.
% Antonio - Y esto? :D
...
GeneBot. However these battles have been performed in 9 maps that were
...
\begin{figure}[ht]
 ...
 \end{center}
 \caption{Boxplot of percentage of victories of the bots obtained by each approach vs GeneBot. 9 different battles have been performed per bot in different maps.} 
...
The boxplots of the percentage of victories from the best bots obtained by each method are shown in Figure \ref{figura:boxplotvictories} and the absolute numbers are shown in Table \ref{tab:allVSall}. It is clear from the image that the Slope fitness does not get good results with respect to the other methods. This could happen due to this approach is the most sensitive to noise, so, maybe the best individuals generated are not actually so good.
...
The Victory-based fitness achieves better results in average, being also more robust (small standard deviation). However, looking at the Area fitness results, they outperform the Victory results, but it is defeated a 50\% more than the Victory. In addition, the Victory-based fitness has more draws that defeats, unlike the other methods.

%Victory-based fitness have a better better behaviour that the other methods. It's more robust, because all the bots obtained by the method have a similar results in the test. However, Area-based fitness reach more wins, but it lost more times that Victory-based fitness. 

%TODO - ¿Por qué es mejor esto?
...
%   \hline 
%  & VICTORIES & AREA & SLOPE & DRAW \\ 
%   \hline \hline
% VICTORIES FITNESS & 12027 & 2617 & 1416 & 7754 \\ 
%   AREA FITNESS & 3483 & 13423 & 2564 & 4344 \\ 
%   SLOPE FITNESS & 5532 & 4382 & 10976 & 2924 \\ 
%    \hline
% \end{tabular}
% \end{table}
...
\begin{tabular}{|r|r|r|r|}
  \hline 
 Fitness Method & Wins & Defeats & Draws \\ 
  \hline \hline
VICTORIES FITNESS & 50.50\% & \textbf{16.94\%} & \textbf{32.56\%} \\ 
  AREA FITNESS & \textbf{56.37\%} & 25.39\% & 18.24\% \\ 
  SLOPE FITNESS & 46.1\% & 41.63\% & 12.27\% \\ 
   \hline
\end{tabular}
 \caption{Percentage of wins, defeats and draws, on test based on each method vs. the rest. Bold numbers indicate the best value.}
 \label{tab:allVSall}
\end{table}
...
%The objective of this work is to validate if using Genetic Programming can create competitive bots for RTS games, and which would be the best fitness function to consider for the evaluation of individuals.
The objective of this work is to demonstrate that using different fitness functions in a Genetic Programming algorithm to automatically create bots that play RTS games  generate different behaviours on these bots.

% study behavior or find which fitness function is better? - JJ
% Nobody is answering my suggestions? - JJ
% Antonio - respondido
With this aim, three different evaluation functions have been compared
in the  generation of bots for the game Planet Wars, namely:
% But you mention in the abstract that you propose fitness functions
% to deal with uncertainty. Have you done so? - JJ
Victory-based, Slope-based and Area-based. For everyone, we have
tested both, their tolerance to noise and their influence in the
quality and structure of the generated bots, i.e. the type of decision
tree that the algorithm generates (conditions and actions). 
...
With respect to the evolutionary process, all the functions seem to work properly, as there is an average improvement in the population along generations, even in the presence of noise. The Victory-based fitness behaves more robustly than the other methods, while the Area-based fitness seems to be less sensitive to noise, and achieves a better percentage of wins in more produced bots but a worse percentage of defeats that Victory-based bots.
...
On the other hand, as Slope-based and Area-based are methods that take into account the number of ships during the run, they promote to attack the planets with a higher growth rate (being enemies or not) and reinforce their own planets more frequently than the Victory-based fitness. 
...
tends to do. 
% Nothing is said about the tree study? Really? It's just there? - JJ
