...
The paper analyses several aspects of the bots, in addition to their final performance on battles, such as the evolution of the behavioural trees themselves, or the influence on the results of noise/uncertainty always present in the context of games.
...
Therefore, the objective of this paper is to generate a reliable BT for a bot, which, according to the state of the map in every simulated turn (input), returns a set of actions to perform in order to fight the enemy, conquer its resources, and finally, win the game. 
...
Among other techniques, Evolutionary Algorithms have been widely used as a  Computational Intelligence method in RTS games, for example, for parameter optimisation \cite{Genebot_CEC11,ExpGenebot_CIG2012}, building order decision \cite{Kostler2013:MO_StarCraftII,Barriga2014:BuildingOrder_GA}, learning \cite{learning_StarCraft_AAIDE11,Wender_RL_CIG12}, or content generation \cite{Mahlmann2012MapGeneration,Lara_EntComp_PCG_RTS14}. 
