...
The paper analyzes several aspects of the bots, in addition to their final performance on battles, such as the evolution of the behavioural trees themselves, or the influence on the results of noise/uncertainty always present in the context of games.
% una ultima frase poniendo el resultado en un ambito general
As a general conclusion a victory-based fitness, which prioritizes the number of victories, contributes to generate better bots, on average, than other functions based on the numerical aspects of the battles, such as the number of resources gathered, or the number of units generated. 
\end{abstract}
...
% Antonio - introdusco aqu� lo que son los EAs, porque viene bien para justificar la importancia del fitness. ;D
...
Therefore, this paper aims to choose the best type of fitness function for a Genetic Programming-based approach, which generates {\em behavioural trees} - or rule-based systems - (BTs), as Artificial Intelligence engines for bots in the scope of RTSs.
To this end, we compare the bots created using different types of fitness, studying both, the generated BTs, and the performance of the bots in battles against unknown enemies. 
...
The rest of the paper is structured as follows: after the background and the problem description (Section \ref{section:background}), the state of the art is presented in Section \ref{sec:soa}. Then, the description of the GP approach and the considered fitness functions are described respectively in Sections \ref{sec:agent} and \ref{sec:fitness_functions}. Section \ref{sec:experiments} explains the experimental setup, while Section \ref{sec:results} analyzes the obtained results. Finally, the conclusions and future lines work are presented in Section \ref{sec:conclusion}.
