...
\title{Testing and Analysing Genetic Programming-based Bots for a Real-Time Strategy Game}
% What problem are we solving? Why do we need to test and analyse? - JJ

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   AUTHORS   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\author{A. Fernández-Ares, A. M. Mora, P. Garcia-Sánchez, P. A. Castillo, J.J. Merelo}
...
% Me refiero a por qué debería GP funcionar mejor que otro tipo de sistema - JJ
This algorithm is applied to design decision trees to be used as bot's AI in one vs one battles inside the RTS game Planet Wars. 
...
Due to the difficulties when applying Computational Intelligence
techniques in the videogames scope, such as uncertainty in the
evaluation functions, several different fitness functions have been
designed and tested in this work. 
% Are they all the possible solutions? Why these three and not 5 or
% only one? - JJ
Two of them try to reduce uncertainty by considering additional dynamic information about the
evaluation matches, rather than just the final result (the winner), as
...
% - Problema 2: paliar el ruido en la evaluación -> intentado resolver
% con nuevas funciones de fitness
%Afú. ¡El problema del ruido en el fitness! -JJ
...
% AntonioDEF - escribir la que realmente sea la mejor, de acuerdo a
% los resultados
% Vamos a ver. ¿Cuál es el selling point del trabajo? ¿Los GPbots o
% cómo enfocáis el problema del ruido? ¿Cómo probáis que los GPbots
% son buenos? ¿Cómo probáis que lo es el abordar el problema del
% ruido? - JJ
\end{abstract}
