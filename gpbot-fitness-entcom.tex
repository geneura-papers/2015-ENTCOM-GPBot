...
which one if less sensitive to this kind of noise. This study would help to other researchers to select the best type of evaluation function for other evolutionary methods working on RTSs.
...
Fitness should have some characteristic in order to ensure a good performance of the algorithm where they are used: 
\begin{itemize}
\item create a smooth landscape: yield similar results in the evaluation of similar individuals.
\item being readily available: every correct individual can always be evaluated.
\item being robust: the evaluation of one individual should be always the same (or similar, at least).
\item being reliable: a good individual (according to the problem objective) must get a higher fitness value that a bad one.
\end{itemize}
% Antonio - estas son las características que hemos pensado, si hay alguna más, por favor, metedla. ;)

This section describes three different fitness functions to be used in the GP algorithm described in Section \ref{sec:agent} in order to generate competitive bots for Planet Wars.
Thus, subsection \ref{subsec:fitness_turns} presents a hierarchical fitness based in the number of victories, and turns of the battles for tie-breaks. 

Subsections \ref{subsec:fitness_slope} and \ref{subsec:fitness_area} show two fitness functions based on the behaviour of the agents during the battles, considering the percentage of resources (ships) gathered by the bot and computing, respectively, a slope or an area as its evaluation value.
...
% Antonio - dicho
This is a variation of the hierarchical fitness considered in \cite{Genebot_CEC11}.
...
In the next subsections there are described the two alternatives to deal with this cloud of points for the fitness function: the use of slopes and areas.
...
\subsection{Actions and conditions analysis}
...
Figure \ref{figura:e_number_nodes} shows the average growing of conditions and actions during the evolution. As described before in Figure \ref{figura:evolutionDEPTH}, when the number of generations increases also does the depth, and therefore the number of nodes. But in this case, in the latter generations of the Victory Fitness the number of nodes increases more than in the other two fitness methods, being the depth of this method also lower than the Slope and Area. This indicates that the Victory Fitness is generating more balanced trees than the other methods.
...
With respect to the decision type, Section \ref{sec:agent} described 6 types of ratios evaluated in each planet to decide which next branch of the tree should be selected. Figure \ref{figura:e_conditions} shows the evolution of these types of ratios. Results show that there is an increase in the Victory Fitness in the rule \emph{actualMyShipsRatio}, the one that compares the number of the ships in the planet executing the tree and the total ships of the player. This makes sense, as the tree is executed for each player's planet and the percentage of ships used at the end of the tree also depends of that current planet. However, with the other two methods, the most used decision compares the number of the players ships. As those fitnesses try to use the number of ships as a quality measure, they are generating trees to maintain higher number of ships during all the match, instead focusing in a quick victory.
...
With respect to the action type that an agent can chose, Figure \ref{figura:e_actions} shows the percentage of the three types of actions that can be performed by the agent: {\em attack}, {\em reinforce} and {\em doNothing}. Results clearly show that the {\em attack} action is the most used in all fitness methods, but it is interesting to mention that the Victory Fitness uses less the {\em reinforcement} one with respect to attacking. This also can be explained because the number of ships is not measured during the evaluation, unlike the other two methods, where a high amount of ships during all the run gives more fitness score.
...
Focusing in the type of attacks, Figure \ref{figura:e_attacks} shows that the \emph{attack_Nearest} action is the most used one in the Victory Fitness. On the contrary, in Slope and Area Fitness, attacking the enemy planets with the highest growing ratio (wealthiest) and the ones with high growing ratio but low number of ships (beneficial) imply less ships for the enemy and more for the player in next turns, and therefore, higher fitness score. However, attacking random planets do not seem to be a good strategy in any of the methods compared. Attacking the base is not a good strategy neither, even it was considered as one of the more important actions in previous works that do not use Genetic Programming \cite{Mora_noisy_jcs}.
...
With respect to the target chosen in every attack, Victory Fitness is again focused in destroy the enemy as quick as possible, avoiding the neutral ones, while the other two methods (Area and Slope) seek any kind of planet to increase their number of ships during the match.
%La figura \ref{figura:e_targets} se presentan los tipos de blancos elegidos al realizar un ataque. Aunque \emph{Target_notMyPlanet} incluye a los otros dos objetivos (\emph{Target_EnemyPlanet} and \emph{Target_NeutralPlanet}, su uso no se incremente a medida que mejoran los individuos. Se puede justificar este comportamiento entendiendo que los planetas neutrales y enemigos requieren mecánicas distintas para ser abordados. El mayor uso de nodos de ataques \emph{Target_EnemyPlanet} en el árbol de decisión de los mejores individuos, muestra que se requieren más reglas para hacer frente al jugador enemigo (que es realmente el oponente y lucha contra nosotros) que para la conquista de planetas neutrales (que no participan activamente en el juego). De igual manera que con el tipo de ataque, el \emph{attack_Random} se mantiene constante.
 ...
 \end{center}
 \caption{Evolution of the percentage of every type of attacking targets over the total of attacks used by the best individual by generation of every execution -- TO DO -- %Se puede ver como existen oscilaciones entre el atacar un planeta enemigo o un planeta neutral
 }
 \label{figura:e_targets}
 \end{figure}

 Finally, and concerning the {\em reinforce} action, the three methods create bots that prefer to reinforce nearest planet, as shown in Figure \ref{figura:e_targetsReinforce}. This is useful, as travelling ships cannot be used as attack or defense, and minimise this time can be more efficient if used to attack instead reinforce distant planets. However, during the evolution of the Victory Fitness a higher oscillation of these percentages can be seen. As previously explained, the bots generated with this method are not focused in the reinforce action, so this number is not maintained during the run. 
...
The results show differences depending on the fitness used: % hey, big
                                % deal - JJ FERGU: reescrito a continuación TODAS las conclusiones
the Victory based fitness create more balanced trees in terms of number of nodes and depth, but the actions of the best individuals obtained are more focused in attack the nearest enemy planets, performing a more aggressive and fast play. On the other hand, the methods that take into account the number of ships during the run try to attack the planets with more growth rate (enemies or not) and reinforce their own planet more times than the Victory fitness. Although the Victory fitness bots win more times against the bot used for training (GeneBot), when confronting all the generated bots between them, the Area fitness wins more times, as there exist a higher variation in depth and number of nodes in the Victory fitness bots, implying more behavioural differences, and maybe, than keeping a high number of ships during all the match implies the bot can resist fast and aggressive attacks.
