...
The automatic generation of autonomous agents (bots) to play this kind of games is also a very prolific research line. However, the applied techniques to do this, must dealt with uncertainty and real-time planning in order to control the game agents.
% una frase muy directa indicando el problema general a resolver en este trabajo
In this work, a genetic programming algorithm to create the behavioural engine of bots able to play a RTS is presented. Three different fitness functions aiming to better deal with this uncertainty are considered, and a detailed analysis on their performance has been conducted. 
% una frase muy directa indicando el resultado obtenido en este trabajo (el mas destacable)
The results show that genetic programming is a suitable method to generate bots to play a 'simple' RTS game, Planet Wars, Moreover it provides a mechanism to generate rules that enable to create very different behaviours.
% un par de frases indicando lo que aporta este resultado al estado del arte
The paper analyzes several aspects of the bots, in addition of their final performance on battles, such as the evolution of the behavioural trees themselves or the influence in the results of noise/uncertainty always present in the context of games.
% una ultima frase poniendo el resultado en un ambito general
As a general conclusion a victory-based fitness, which prioritizes the number of victories, contributes to generate better bots, on average, than other functions based on numerical aspects of the games (such as the number of resources gathered, or the number of units generated). 
\end{abstract}
